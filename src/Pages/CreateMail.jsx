        ...
        <div className="bg-[#0c072c] lg:p-8 relative py-8 px-5 rounded-2xl w-full max-w-[35rem] border-[0.1px] border-[#453995]">
          ...
          <div className="bg-[#161134] rounded-xl flex items-center justify-between p-5 relative">
            <button onClick={handleDecrement}>
              <FontAwesomeIcon icon={faMinus} className='py-3 px-3 rounded-full text-[#0c072c] font-bold bg-[#dddddd] active:bg-[#142c5f] active:text-[#fff]' />
            </button>
            <p className='text-white text-2xl font-semibold'><span>{years}</span> Year{years > 1 ? 's' : ''}</p>
            <button onClick={handleIncrement}>
              <FontAwesomeIcon icon={faPlus} className='py-3 px-3 rounded-full text-[#0c072c] font-bold bg-[#3C77FB] active:bg-[#142c5f] active:text-[#fff]' />
            </button>
          </div>
          ...
          <button className='text-[#ffff] bg-[#3C77FB] hover:bg-[#2b5ac0] active:bg-[#142c5f] rounded-3xl w-full p-5 text-xl' onClick={handleSuccess}>
            {loading ? (
      ...
      {successPop && (
        <div className="p-12 rounded-2xl bg-white lg:w-[500px] w-[350px] flex flex-col justify-center items-center">
          ...
          <p className='text-[#3C77FB] lg:text-2xl text-lg font-semibold text-center'>
            Mail Created Successfully and it's Valid for <span>{years}</span> <span>{years < 2 ? 'year' : 'years'}</span>
          </p>
        </div>
      )}
    </div>
